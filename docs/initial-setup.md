...
[Step 1] USB Serial 接続を試す
      │
      ├─ 応答あり (出荷時にファームが入っている場合)
      │     → そのまま Step 3 へ
      │
      └─ 応答なし (新品 / ブートローダー空 / ダウンロードモード)
            │
            ▼
       [Step 2] ファームウェア書き込み
            │
...
[Step 3] Wi-Fi / 名前 / グループ設定
      │
      ▼
[完了] デバイスが LAN に乗り、Studio サイドバーに表示される
```
...
5. **書き込み完了**: ウィザードが自動的に「再起動を待機中… (残り 6 秒)」をカウントダウンし、Step 1 の接続試行を再実行します。
   - 6 秒後に Step 1 の接続成功 → Step 3 に自動遷移すれば成功です。
